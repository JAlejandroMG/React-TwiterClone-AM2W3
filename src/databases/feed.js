const feed = [
  {
    profileUrl: "https://img.favpng.com/24/25/17/al-jazeera-media-network-aj-logo-vector-graphics-png-favpng-EqmhvQLUYjTHfV9ksryKQjSwV_t.jpg",
    profile: "AJ + Español",
    username: "ajplusespanol",
    content: `Laika fue elegida para viajar al espacio sabiendo que no resistiría.
    ¿Son éticas nuestras maneras de hacer ciencia? 
    Queremos saber qué opinas.`,
    interaction: [
      {selectedComments: false, comments: 5},
      {selectedRetweets: false, retweets: 66},
      {selectedLikes: false, likes: 126}
    ],
    verified: false,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://www.biography.com/.image/ar_1:1%2Cc_fill%2Ccs_srgb%2Cg_face%2Cq_auto:good%2Cw_300/MTY2MzU3Nzk2OTM2MjMwNTkx/elon_musk_royal_society.jpg",
    profile: "Elon Musk",
    username: "elonmusk",
    content: `Several thousand more Starlink beta participation invitations going out this week.`,
    interaction: [
      {selectedComments: false, comments: 6756},
      {selectedRetweets: false, retweets: 3734},
      {selectedLikes: false, likes: 84034}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://grandesmarcas.cl/wp-content/uploads/2018/06/Logo-Google.jpg",
    profile: "Google",
    username: "google",
    content: `20 years ago today the first crew of astronauts and researchers arrived at the International Space Station, forever changing how we explore space. Cohete Celebrate #SpaceStation20th with @NASA & @GoogleArts by exploring a 3D model of the`,
    interaction: [
      {selectedComments: false, comments: 26},
      {selectedRetweets: false, retweets: 100},
      {selectedLikes: false, likes: 693}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://www.earnthenecklace.com/wp-content/uploads/2017/06/lebron-james-net-worth-300x300.jpg",
    profile: "LeBron James",
    username: "KingJames",
    content: `Y’all see the setup right???!!!`,
    interaction: [
      {selectedComments: false, comments: 17615},
      {selectedRetweets: false, retweets: 1382},
      {selectedLikes: false, likes: 78803}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://pbs.twimg.com/profile_images/1150716997254209536/M7gkjsv5.jpg",
    profile: "BBC Breaking News",
    username: "BBCBreaking",
    content: `Prince William contracted Covid-19 in April at a similar time to his father Prince Charles, palace sources tell BBC`,
    interaction: [
      {selectedComments: false, comments: 1315},
      {selectedRetweets: false, retweets: 2200},
      {selectedLikes: false, likes: 4203}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://bitcoin.es/wp-content/uploads/2018/12/Wcem1g-S_400x400-300x300.jpg",
    profile: "NVIDIA GeForce",
    username: "NVIDIAGeForce",
    content: `Hitch a ride on the RTX Battle Bus!
    Experience Fortnite in a new light with NVIDIA ray traced shadows, global illumination and ambient occlusion. 
    Learn more about RTX, DLSS, and NVIDIA Reflex in Fortnite: https://nvidia.com/en-us/geforce/campaigns/play-fortnite-with-rtx/`,
    interaction: [
      {selectedComments: false, comments: 56},
      {selectedRetweets: false, retweets: 34},
      {selectedLikes: false, likes: 524}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://www.theimagingalliance.com/wp-content/uploads/national-geographic-imagingalliance-member-logo-300x300.png",
    profile: "National Geographic",
    username: "NatGeo",
    content: `What's the most soothing experience you've had in nature? (Bonus points if you have a photo or video)`,
    interaction: [
      {selectedComments: false, comments: 128},
      {selectedRetweets: false, retweets: 76},
      {selectedLikes: false, likes: 434}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://is3-ssl.mzstatic.com/image/thumb/Podcasts113/v4/9e/4f/3a/9e4f3abb-e6c8-178e-c759-1a00a1da5a8c/mza_4670939643605397174.jpg/300x300bb.jpg",
    profile: "Hipertextual",
    username: "Hipertextual",
    content: `Cinco recomendaciones, y una sorpresa, para ver en Netflix en noviembre`,
    interaction: [
      {selectedComments: false, comments: 0},
      {selectedRetweets: false, retweets: 1},
      {selectedLikes: false, likes: 4}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://www.marketingdirecto.com/wp-content/uploads/2011/11/real-madrid-300x300.jpg",
    profile: "Real Madrid C.F.",
    username: "realmadrid",
    content: `¡El capitán, @SergioRamos, alcanza la cifra de 1️⃣0️⃣0️⃣ goles con la camiseta del 
    @RealMadrid! #RealFootball | #HalaMadrid`,
    interaction: [
      {selectedComments: false, comments: 210},
      {selectedRetweets: false, retweets: 3700},
      {selectedLikes: false, likes: 14900}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://i2.wp.com/www.negociosmagazine.com/wp-content/uploads/2016/12/uefa_champions_league-e1481719828492.jpg?resize=300%2C300&ssl=1",
    profile: "UEFA Champions League",
    username: "ChampionsLeague",
    content: `Bayern have won their last 13 Champions League games... #UCL`,
    interaction: [
      {selectedComments: false, comments: 19},
      {selectedRetweets: false, retweets: 54},
      {selectedLikes: false, likes: 818}
    ],
    verified: true,
    blocked: false,
    showContextM: false
  },
  {
    profileUrl: "https://static.dezeen.com/uploads/2018/10/Barcelona-FC-redesign_dezeen_sq1-300x300.jpg",
    profile: "FC Barcelona",
    username: "FCBarcelona_es",
    content: `¡La 
    @LigadeCampeones no se detiene!
    Preparados para una nueva noche europea. 
    Este miércoles, #BarçaDynamo`,
    interaction: [
      {selectedComments: false, comments: 6},
      {selectedRetweets: false, retweets: 30},
      {selectedLikes: false, likes: 723}
    ],
    verified: true,
    blocked: true
  },
  {
    profileUrl: "https://rigocaro.com/wp-content/uploads/2018/07/youtube-300x300.png",
    profile: "Youtube",
    username: "Youtube",
    content: `Drop links to the videos that are helping you right now`,
    interaction: [
      {selectedComments: false, comments: 4900},
      {selectedRetweets: false, retweets: 878},
      {selectedLikes: false, likes: 11300}
    ],
    verified: true,
    blocked: true
  }
];


export {
  feed
};
